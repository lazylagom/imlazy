...
| 상황 | 판단 기준 | 대응 |
|------|-----------|------|
| 진입점 없음 | 관련 키워드 3회 검색 후 무결과 | 사용자에게 힌트 요청 |
| 너무 복잡 | 의존성이 10개+ 파일로 분산 | THEORIZE로 가설 세우고 부분 탐색 |
| 문서 부재 | 코드에 주석/문서 없음 | 런타임 테스트로 동작 확인 |
...
**탐색 예산**: Haiku 모델은 빠르지만 깊은 추론에 약함. 3회 이상 막히면 THEORIZE(Opus)로 가설 기반 탐색 전환.
